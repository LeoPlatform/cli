...
function apply(config, override, path, triggers, depth) {
...

	{
		var obj = Object.keys(override).filter(k =>
			...
			if (typeof override[k] === "object" && override[k] != undefined && !Array.isArray(override[k])) {
				o[k] = {};
				apply(o[k], override[k], path, triggers, depth + " ")
			} else if (Array.isArray(override[k])) {
				o[k] = [];
				let t = {};
				override[k].forEach((v, i) => t[i] = v);
				apply(o[k], t, path, triggers, depth + " ")
			} else {
				o[k] = override[k];
			}
			return o;
	...
	for (var key in node) {
		...
		if (child && typeof(child) == "object" && Object.keys(child).length == 1 && Object.keys(child)[0].toLowerCase() == "ref") {
			...
			if ((typeof val == "undefined" || val == null) && !currentOpts["ignore-missing-variables"]) {
				...
				throw new Error(`Reference Variable "${ref}" is required`);
			}
...
module.exports = {
	...
	"build": function build(rootDir, opts, pkgOverride) {
		...
		} else if (process.env.LEO_INCLUDE_LOCAL == "true") {
			overrideFiles.push({
				type: "system",
				postfix: "-local-" + env
			});
		...
		overrideFiles.forEach(function(c) {
			...
			// Apply the current level config
			// console.log("Apply", c.type, configs[c.type], override)
			apply(config, configs[c.type], objPath.length ? objPath : null, triggers, "");

			// Apply the current level config's overrides
		...
		config._meta = {
			env,
			region,
			configDir: systemDirectory && path.resolve(systemDirectory, "config/"),
