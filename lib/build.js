...
function writeCloudFormation(rootDir, opts, program, config, callback) {
	...
	if (cf) {
		let now = (opts.info && opts.info.build) || Date.now();
		...
		if (config._meta && config._meta.microserviceDir){
			let onSaveHook = path.resolve(config._meta.microserviceDir, "build-hooks/on-cloudformation-save.js");
			if (fs.existsSync(onSaveHook)){
				require(onSaveHook)(cf, opts.info);
			}
		}
