...
module.exports = {
	build: function(program, rootDir, opts = {}, callback) {
		...
		if (config.type === "bot" || config.type === "cron" || config.type === "resource" || config.type === "apigateway") {
			...
			if (!opts.cloudFormationOnly) {
				...
				console.log("\n\n---------------Building Lambda-------------------\n\n");
				async.mapLimit(opts.lambdas, config.parallelCompile || 5, (lambdaDir, done) => {
	...
	publish: function(rootDir, remoteDir, opts, callback) {
		...
		if (opts.profile) {
			let credentials = require("./leo-aws")(opts.profile);
			// If credentials is a static object (from cached STS), use directly
			// If it's a provider function (fromIni), resolve it
			let resolveAndUpload = async () => {
				let resolved = typeof credentials === 'function' ? await credentials() : credentials;
				let env = {
			...
			resolveAndUpload().catch(err => callback(err));
		} else {
