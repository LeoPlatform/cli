...
async function promptDefault(p, def) {
	let a = (await prompt(p + `[${def}]: `)).trim();
	if (a == "") {
		a = def;
	}
	return a;
...
module.exports = async function() {
	let environments = await promptDefault('Which environments would you like to setup? (comma delimited)', "NONE");
	...
	for (var i = 0; i < environments.length; i++) {
		...
		console.log(`----------------${env}------------`);
		let region = await promptDefault(`In which aws region is ${env} located?`, 'us-west-2');
		let profile = await promptDefault(`Which aws cli profile does ${env} use?`, 'default');
	...
	async function getStackProfileId(questionText, env, region, profile, postFix) {
		let response = await promptDefault(questionText, `${capitalizeFirstLetter(env)}${postFix}`);
		let result;
