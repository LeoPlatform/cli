...
module.exports = function(argv, callback) {
	...
	program
		...
		.action(async function(dir, testName) {
			...
			if (config._meta && config._meta.microserviceDir) {
				let msName;
				try {
					msName = require(path.resolve(config._meta.microserviceDir, "package.json")).name;
				} catch (e) {
					// Blank
				}
				if (config._meta.systemDir) {
					paths.push(path.resolve(config._meta.systemDir, "test"));
					paths.push(path.resolve(config._meta.systemDir, "config"));
					msName && paths.push(path.resolve(config._meta.systemDir, `../test/${msName}`));
					msName && paths.push(path.resolve(config._meta.systemDir, `../config/${msName}`));
				}
				paths.push(path.resolve(config._meta.microserviceDir, "test"));
				msName && paths.push(path.resolve(config._meta.microserviceDir, `../test/${msName}`));
				msName && paths.push(path.resolve(config._meta.microserviceDir, `../config/${msName}`));
			}
			paths.push(path.resolve(rootDir, "test"));

			let envVars = Object.assign({}, config.env);
