...
module.exports = {
	createCloudFormation: function(dir, opts) {
		return new Promise((resolve, reject) => {
			...
			this.getBuckets(opts.targets, {}, (err, buckets) => {
				...
				if (opts.cloudformation) {
					...
					cloudFormation.Parameters = Object.assign({
						"Environment": {
							"Type": "String",
							"Default": "dev",
							"MinLength": 1,
					...
					}, cloudFormation.Parameters, microservice.config && microservice.config.leo && microservice.config.leo.Parameters || {});
				...
				glob(path.resolve(path.resolve(dir, "*(bots|api)/{,!(node_modules)/**/}" + `/${opts.filter}/package.json`)), {
					...
					processFiles.map((file) => {
						...
						const ID = package.logicalResource || utils.properCaseTransform(package.name);
						...
						let entryData = createLambdaEntry(existing, package, newPath, file, cloudFormation.Parameters);
						if (entryData) {
						...
						addStacks(package, cloudFormation, opts.linkedStacks)
					...
					if (microservice.config && microservice.config.leo && microservice.config.leo.subtype) {
						...
						if (opts.force === "all" ||
							...
							}, newPath, cloudFormation.Parameters);
	...
	getBuckets: function(targets, opts, callback) {
		...
		targets.forEach(target => {
			tasks.push(function(done) {
				...
				cloudformation.describeStackResources(opts.name).catch(err => {
					if (err.message == `Stack with id ${opts.name} does not exist`) {
						...
						return cloudformation.createStack(opts.name, require("../cloudformation.json"), [], true)
							.then(() => cloudformation.describeStackResources(opts.name))
							.catch(err => {
...
function createLambdaEntry(existing, properties, newPath, file, parameters = {}) {
...
function addStacks(package, cloudFormation, additionalStacks) {
