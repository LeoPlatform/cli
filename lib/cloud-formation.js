							...
							dir, {
								...
								if (!err && data) {
									...
									if (fs.existsSync(publishjs)) {
										tasks.push(done => require(publishjs)(buildDir, cloudFormation, done, { build: data, version, tag: opts.tag, name: microservice.name, s3Folder: `/${microservice.name}${opts.tag}/${version}` }));
									}
									let deployedStatic = {};
