...
function getBuckets(regions, opts, callback) {
	...
	regions.forEach(region => {
		tasks.push(function(done) {
			let cloudformation = region.cloudformation;
			cloudformation._service.describeStackResources({
				StackName: opts.name
			}).promise().catch(err => {
				if (err.message == `Stack with id ${opts.name} does not exist`) {
					...
					return createStack(opts.name, require("../cloudformation.json"), region).catch(err => {
...
async function createStack(name, template, region, opts = {}) {
	var overridePrompts = {
	...
	var cloudformation = region.cloudformation;
