				...
				glob(path.resolve(path.resolve(dir, "*(bots|api)/{,!(node_modules)/**/}" + `/${opts.filter}/package.json`)), {
					...
					glob(path.resolve(path.resolve(dir, "*cloudformation/{,!(node_modules)/**/}" + `/*.js`)), {
						...
						files.forEach(file => {
							...
							merge(cloudFormation, {
								Resources: contents.export ? contents.export() : contents
							});
						});
			...
			tasks.push(function(done) {
				...
				cloudformation.describeStackResources(opts.name).catch(err => {
					if (err.message == `Stack with id ${opts.name} does not exist`) {
						...
						return cloudformation.createStack(opts.name, require("../cloudformation.json"), region).catch(err => {
