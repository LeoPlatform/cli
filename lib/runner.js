...
let argv = (process.send && !process.env.runner_keep_cmd) ? process.argv.slice(2) : process.argv;
...
require("./leo-bot-run.js")(argv, (err, module) => {
	let doCall = () => {
		...
		let r = module.handler(module.event, createContext({}, module.config), cb);
		if (r && r.then) {
			r.then((d) => cb(null, d)).catch(cb);
		}
	};
